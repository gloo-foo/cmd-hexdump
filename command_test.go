...
func TestHexdump_UnknownOptionPassesThrough(t *testing.T) {
	// An argument that is not one of hexdump's option types is handed to the
	// framework's positional classifier and must not disturb rendering.
	lines, err := testable.TestLines(command.Hexdump(42), "hi\n")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(lines) != 1 || lines[0] != "68 69" {
		t.Fatalf("expected [68 69], got %v", lines)
	}
}
